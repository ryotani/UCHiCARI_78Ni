...
int Everesp[8]={100000,100000,100000,100000,100000,100000,100000};
//
int Enep2p[]={5293, 3183, 5243, 3703};
int life4=0;
int Evep2p[]={765, 364, 655, 473};
int Enep3p[]={5293, 3977, 2910};
int Evep3p[]={242, 307, 557};
//
...
int loadsim(void){
  ...
  //cout<< plot()<<endl;;
  //int numbin = 200;
  h_Actual[0] = new TH1F("h_Actual0", "Actual; Energy / keV; Counts / 10 keV", numbin, min, max);
  h_AddATC[0] = new TH1F("h_AddATC0", "AddATC - MB; Energy / keV; Counts / 10 keV", numbin, min, max);
  h_AddATC_SCout[0] = new TH1F("h_AddATC_SCout0", "ATC - SC; Energy / keV; Counts / 10 keV", numbin, min, max);
  h_Actual[1] = new TH1F("h_Actual1", "; Energy / keV; Counts / 10 keV", numbin, min, max);
  h_AddATC[1] = new TH1F("h_AddATC1", "; Energy / keV; Counts / 10 keV", numbin, min, max);
  h_AddATC_SCout[1] = new TH1F("h_AddATC_SCout1", "; Energy / keV; Counts / 10 keV", numbin, min, max);
  
  //for(int i=0; i<1; i++){ //p2p
  for(int i=0; i<4; i++){ //p2p
    TH1F *htmp1 = new TH1F("htmp1", "; Energy / keV; Counts / 10 keV", numbin, min, max);
    TH1F *htmp2 = new TH1F("htmp2", "; Energy / keV; Counts / 10 keV", numbin, min, max);
    TH1F *htmp3 = new TH1F("htmp3", "; Energy / keV; Counts / 10 keV", numbin, min, max);
  
    draw_comparison(Form("root/p2p_%ikeV_%ievt_0ps.root", Enep2p[i], Evep2p[i]), 0, htmp1, htmp2, htmp3);
    if(i==0){
  ...
  for(int i=0; i<3; i++){ //p3p
    ...
    TH1F *htmp2 = new TH1F("htmp2", "; Energy / keV; Counts / 10 keV", numbin, min, max);
    TH1F *htmp3 = new TH1F("htmp3", "; Energy / keV; Counts / 10 keV", numbin, min, max);
    draw_comparison(Form("root/p3p_%ikeV_%ievt_0ps.root", Enep3p[i], Evep3p[i]),0, htmp1, htmp2, htmp3);
    if(i==0){
      /*
...
int loadresp(void){
  for(int i=0; i<7; i++){
    TH1F *hresptemp[3];
    for(int j =0; j<3; j++){
      hresptemp[j] = new TH1F(Form("hresptmp%i",j), "2910 keV with AB (Black: Actual geometetry, Red: + ATC - 2 MB); Energy / keV; Counts / 10 keV", numbin, min, max);
    }
    draw_comparison(Form("root/p%ip_%ikeV_%ievt_%ips.root", i<5?2:3,Eneresp[i],Everesp[i],0), 0, hresptemp[0], hresptemp[1], hresptemp[2]);
    for(int j =0; j<3; j++){
    ...
    delete hresptemp[1];
    delete hresptemp[2];
    cout<<"Loaded: "<<Form("root/p%ip_%ikeV_%ievt_%ips.root", i<5?2:3,Eneresp[i],Everesp[i],0)<<endl;
  }
  ...
  for(int i=0; i<6; i++){
    whole[i/2][i%2]->SetNpx(5000/((max-min)/numbin));
  }
  //
  return 0;
}
