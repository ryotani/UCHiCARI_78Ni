...
ofstream fout(Form("fig/p2pp3p_ATC_Study_%ikeV.csv",bin));
...
int loadsim(void){
  ...
  for(int i=0; i<4; i++){ //p2p
    ...
    draw_comparison(Form("files/6.8mm-Actual/root/p2p_%ikeV_%ievt_0ps.root", Enep2p[i], Evep2p[i]), 0, htmp1, htmp2, htmp3);
    if(i==0){
  ...
  for(int i=0; i<3; i++){ //p3p
    ...
    TH1F *htmp2 = new TH1F("htmp2", "; Energy / keV; Counts / 20 keV", numbin, 200, 4200);
    TH1F *htmp3 = new TH1F("htmp3", "; Energy / keV; Counts / 20 keV", numbin, 200, 4200);
    draw_comparison(Form("files/6.8mm-Actual/root/p3p_%ikeV_%ievt_0ps.root", Enep3p[i], Evep3p[i]),0, htmp1, htmp2, htmp3);
    if(i==0){
      /*
...
int loadresp(void){
  for(int i=0; i<7; i++){
    ...
    draw_comparison(Form("files/6.8mm-Actual/root/p%ip_%ikeV_%ievt_%ips.root", i<5?2:3,Eneresp[i],Everesp[i],0), 0, hresptemp[0], hresptemp[1], hresptemp[2]);
    for(int j =0; j<3; j++){
    ...
    delete hresptemp[1];
    delete hresptemp[2];
    cout<<"Loaded: "<<Form("files/6.8mm-Actual/root/p%ip_%ikeV_%ievt_%ips.root", i<5?2:3,Eneresp[i],Everesp[i],0)<<endl;
  }
