...
int life4=0;
int Eve[nn]={765, 364, 655, 473}, bin=40, ymax=82; char *output=Form("fig/ggp2p_78Ni_10d_Sep2019_%ips",life4);
...
int gatemin[3]={553, 1023, 2500};
int gatemax[3]={613, 1183, 2700};
int histnumindex[3]={0,1,4};
...
int drawhist(int hn){
  ...
  for(int i=0;i<nn;i++){
    TH2F* h_tmp = (TH2F*)(f[i]->Get("egamegamABdc"));//Get(hist[0]));// hist[hn=0] <-> egamgamABdc
    //cout<<h_tmp;
    if(i==0){
  ...
  h[hn][nn]->SetTitle(Form("Gate on %i keV",(gatemin[hn]+gatemax[hn])/2));
  h[hn][nn]->Rebin(bin/4);
  ...
  h[hn][nn]->Draw();
  //whole[0]->FixParameter(5,0);
  whole[0]->FixParameter(7,0);
  whole[0]->FixParameter(histnumindex[hn],0);
  h[hn][nn]->Fit(whole[0],"WQ","",100,3800);
  h[hn][nn]->Fit(whole[0],"LM","",100,3800);
  /*
  ...
  l->Draw();
  */
  whole[0]->SetParLimits(histnumindex[hn],0,100);
}
